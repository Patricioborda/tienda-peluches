...
function PeluchesPage() {
  ...
  const handleDelete = async (id, nombre) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: "¡Eliminado!",
          text: "El peluche ha sido eliminado correctamente",
          icon: "success",
          timer: 2000,
  ...
  // Función para filtrar y ordenar peluches
  const getFilteredAndSortedPeluches = () => {
    let filtered = peluches.filter(peluche =>
      ...
      peluche.descripcion.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (peluche.categoria?.nombre || "").toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        <div className="header-content">
          <div className="header-text">
            ...
            <p className="page-description">
              Administra tu inventario de peluches de forma fácil y eficiente
            </p>
          </div>
          <button 
      ...
      <div className="stats-container">
        <div className="stat-card">
          ...
          <div className="stat-info">
            <span className="stat-number">{peluches.length}</span>
            <span className="stat-label">Total Peluches</span>
          </div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">🔢</div>
        ...
        <div className="stat-card">
          <div className="stat-icon">💰</div>
          <div className="stat-info">
            <span className="stat-number">${valorTotal.toFixed(2)}</span>
            <span className="stat-label">Valor Inventario</span>
          </div>
        </div>
        <div className={`stat-card ${stockBajo > 0 ? 'warning' : ''}`}>
          <div className="stat-icon">⚠️</div>
          <div className="stat-info">
            <span className="stat-number">{stockBajo}</span>
      ...
      {filteredPeluches.length === 0 ? (
        <div className="empty-state">
          ...
          <h3 className="empty-title">
            {searchTerm ? "No se encontraron peluches" : "No hay peluches registrados"}
          </h3>
          <p className="empty-description">
            {searchTerm 
              ? "Intenta con otros términos de búsqueda"
              : "¡Comienza agregando tu primer peluche a la colección!"
            }
          </p>
          {!searchTerm && (
            <button 
            ...
            <div key={peluche.id} className="peluche-card">
              <div className="card-image-container">
                <img
                  ...
                  onError={(e) => {
                    const handleImageError = (e) => {
                      ...
                      // Usar imagen SVG inline como fallback
                ...
                <div className={`stock-badge ${
                  (peluche.stock || 0) > 10 ? 'high' : 
                  (peluche.stock || 0) > 5 ? 'medium' : 
                  (peluche.stock || 0) > 0 ? 'low' : 'empty'
                }`}>
              ...
              <div className="card-content">
                ...
                <p className="card-description">
                  {peluche.descripcion || "Sin descripción"}
                </p>
                ...
                <div className="card-actions">
                  ...
                  <button
                    onClick={() => handleDelete(peluche.id, peluche.nombre)}
      ...
      {showModal && (
        <CreatePeluchePage
          ...
          onSuccess={() => {
            ...
            Swal.fire({
              title: "¡Éxito!",
              text: "Peluche creado correctamente",
              icon: "success",
              timer: 2000,
              showConfirmButton: false
            });
